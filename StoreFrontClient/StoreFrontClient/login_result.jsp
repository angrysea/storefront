...
<%
    try {
        ...
        if(loginResponse.getuser() != null && loginResponse.getuser().getloggedin())
        {
            ...
            // WE HAD A BUG IN PRODUCTDETAILS WITH THIS
            // response.sendRedirect(request.getParameter("gotourl"));

            // CHANGED TO
            response.sendRedirect(StoreFrontUrls.getsecureurl(request, company, "youraccount"));
        }
        else
        {
            // failed, go back to the login screen
            response.sendRedirect(StoreFrontUrls.getsecureurl(request, company, "login",
            	request.getParameter("gotourl") + "&error=Invalid e-mail address or password."));
        }
    }
    catch (Exception ex) {
        ex.printStackTrace();
