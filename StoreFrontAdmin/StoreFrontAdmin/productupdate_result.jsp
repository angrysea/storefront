...
<%
    ...
    try {
        ...
        Item item = getItemResponse.getitem();

	// Store the item information
        UpdateItemRequest updateItemRequest = new UpdateItemRequest();
        item.setid(new Integer(request.getParameter("id")).intValue());
	...
	updateItemRequest.setitem(item);
        updateItemResponse = itembean.UpdateItem(updateItemRequest);
    }
    catch (Exception ex) {
        ex.printStackTrace();
            ...
            <tr>
                ...
                <td vAlign="top">
                    <table id="headingTable" border="0">
                        ...
                        <tr>
                            <td class="producttitle">Update Product Result</td>
                        </tr>
                    </table>
                    <br>
                    <TABLE cellSpacing="1" cellPadding="3" width="95%" border="0">
                    <tr>
                        <td noWrap>The product [<%=request.getParameter("productname")%>] was updated successfully.</td>
                    </tr>
                    <tr>
                        <td noWrap><a href="./productupdate.jsp?id=<%=new Integer(nextItem).toString()%>&manufacturer=<%=request.getParameter("manufacturer")%>">next product >></a></td>
                    </tr>
                    </TABLE>
                </td>
            </tr>
