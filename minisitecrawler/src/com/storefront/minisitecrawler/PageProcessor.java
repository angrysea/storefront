...
public class PageProcessor extends DefaultHandler {
  ...
  private static String dbUrl =
      "jdbc:mysql://10.1.1.50:3306/crawler?user=crawler&password=crawler";

  private BufferedOutputStream os = null;
  private String baseDirectory = null;
  private String baseurl = null;

  public PageProcessor(MiniSiteCrawler crawler, String baseDirectory, String baseurl) {
  ...
  public String generatename(String url) throws java.
    ...
    if(len>jspat+4)
      filename += url.substring(jspat+4);
    ...
    for (int i = 0; i < len; i++, j++) {
      while (i < len && (chars[i]==' '||
          (!Character.isDigit(chars[i])&&!Character.isUpperCase(chars[i])&&!Character.isLowerCase(chars[i])))) {
        i++;
      }
      if(i < len)
        chars[j] = chars[i];
